    ...
    public class OpenAIModelProviderService : IModelProviderService
    {
            ...
            CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                // 筛选和排序模型
                var filteredModels = FilterAndSortModels(modelResponse.Data);

                _logger.ZLogInformation($"成功获取 {filteredModels.Count} 个模型");
        ...
        /// 筛选和排序模型列表
        /// </summary>
        private List<ModelInfo> FilterAndSortModels(List<ModelInfo> models)
        {
            // 筛选出聊天相关的模型
            var filtered = models
                .Where(m => 
                    // GPT系列模型
                    m.Id.Contains("gpt", StringComparison.OrdinalIgnoreCase) ||
                    // 包含chat关键字
                    m.Id.Contains("chat", StringComparison.OrdinalIgnoreCase) ||
                    // OpenAI官方模型
                    (m.OwnedBy?.Equals("openai", StringComparison.OrdinalIgnoreCase) == true &&
                     !m.Id.Contains("whisper", StringComparison.OrdinalIgnoreCase) &&
                     !m.Id.Contains("tts", StringComparison.OrdinalIgnoreCase) &&
                     !m.Id.Contains("dall-e", StringComparison.OrdinalIgnoreCase) &&
                     !m.Id.Contains("embedding", StringComparison.OrdinalIgnoreCase)))
                .ToList();

            // 排序：按模型名称排序
            var sorted = filtered
                .OrderByDescending(m => m.Created) // 先按创建时间降序
                .ThenBy(m => m.Id)                 // 再按ID升序
                .ToList();

            _logger.ZLogDebug($"筛选后的模型数量: {sorted.Count}");
